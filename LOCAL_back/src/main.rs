...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(|| {
        App::new()
            ...
            // .service(Files::new("/dist", "../copied/dist"))
            // .service(Files::new("/", "../copied/index.html").index_file("index.html"))
            .service(
                spa() // now runable from root, just ofc compile frontend
                // .index_file("./front/dist/index.html") 
                // .static_resources_location("./front/dist")
                .index_file("../copied/index.html") 
                // .static_resources_mount("/dist")
                .static_resources_location("../copied/dist")
                .finish()
            )
    })
    .bind(("0.0.0.0",8080))?
